...
### Fixed: Identity Response Cascade (Rate Limiting)

- **`_register_success` called before identity check — failure counter neutralized** (`translator.py`): When Google returned a 200 response, `_register_success(ep)` was called immediately (decrementing fails by 1), and then the identity check ran and called `_register_failure(ep)` (incrementing fails by 1). Net effect: **zero** — identity responses never accumulated failures, so endpoints were **never banned** during an identity cascade. Moved `_register_success` to after the identity check passes.
- **`_register_success` decremented fails by 1 instead of resetting** (`translator.py`): A single successful request after 4 failures only reduced the counter to 3, creating persistent "failure debt" that accumulated across transient issues. Changed to full reset (`fails = 0`) on success.
- **No per-attempt backoff for identity responses** (`translator.py`): Hard 429 responses triggered exponential backoff (`2^attempt + jitter`), but identity responses (soft 429) had no per-attempt delay — they immediately retried after the flat 10s global cooldown. Added the same exponential backoff pattern for identity responses.
- **Global cooldown flat and non-escalating** (`translator.py`): The `max()` logic meant cascading identity responses within the same 10s window didn't extend the cooldown. Replaced with escalating cooldown: `10s × 2^(consecutive-1)`, capped at 60s. Five consecutive identities across all mirrors now produce a 10s → 20s → 40s → 60s → 60s sequence.
- **Concurrent coroutines bypassed circuit breaker** (`translator.py`): With 16 concurrent `process_slice` tasks, coroutines that were already past the cooldown check when the circuit breaker tripped continued making requests, producing cascading identity counts (5→6→7→…→10+). Fixed with three mechanisms: (1) circuit breaker gate check at the top of every retry attempt — returns `None` immediately instead of retrying; (2) chunked cooldown wait (3s chunks with jitter) that re-checks `_global_cooldown_until` for extensions by other coroutines; (3) post-cooldown re-check catches breakers tripped during sleep. Circuit breaker now logs only once (not per-coroutine) and bails out all concurrent coroutines.
- **Thundering herd on cooldown wake-up** (`translator.py`): All concurrent coroutines sleeping on the same `_global_cooldown_until` timestamp woke simultaneously and fired requests in lockstep. Added `random.uniform(0.1, 0.5)` jitter to each wake-up cycle and `random.uniform(0, 0.05)` to request delays, spreading coroutine wake-ups over a ~400ms window.
- **Individual retry amplifies load during rate-limit** (`translator.py`): When a batch translation failed, the system spawned N parallel individual retries via `asyncio.gather`. During an identity cascade, this multiplied the request count, worsening the rate-limit. Individual retries are now skipped when the circuit breaker is active.
- **Lingva responses not checked for identity** (`translator.py`): Lingva fallback accepted responses blindly — if a Lingva mirror also echoed text unchanged, it was treated as a valid translation. Added identity check for Lingva responses; successful Lingva translations reset the identity cascade tracker. Lingva fallback is also skipped during active circuit breaker cooldown.
...
- **Batch separator ASCII migration** (`constants.py`, `translator.py`, `text_merger.py`): Unicode separator tokens `⟦_S_⟧` / `⟦_M_⟧` / `⟦_B_⟧` replaced with ASCII equivalents `|||RPGMSEP_S|||` / `|||RPGMSEP_M|||` / `|||RPGMSEP_I|||`. Google was silently converting `⟦` and `⟧` to `?`, causing every batch split to fail and the pipeline to fall back to per-item retries for every request. `REGEX_*` split patterns updated with ASCII primary + legacy Unicode fallback so in-progress translation caches remain compatible.
- **`"format": "text"` parameter removed** (`translator.py`): `client=gtx` endpoint does not recognise this parameter; some mirrors returned identity responses when it was present. Removed.
- **`_get_next_endpoint` IndexError hardened** (`translator.py`): `_endpoint_index` is global state but `available` is rebuilt each call with a variable-size subset of healthy endpoints. When the pool shrank (banned endpoints removed), the stale index could exceed `len(available)`, raising `IndexError`. Now clamped with `min(self._endpoint_index, len(available) - 1)` before the modulo increment.
- **Batch `parts` overflow trim always applied** (`translator.py`): The guard `if len(parts) > expected_count and not parts[-1]` was dead code — the preceding `if p.strip()` filter ensures no empty strings survive, so `not parts[-1]` is always `False`. Replaced with an unconditional `parts = parts[:expected_count]` trim so genuinely over-split batches are recovered instead of failing with a mismatch error.
- **Lingva exception now logged** (`translator.py`): `except Exception` in the Lingva fallback loop swallowed all errors silently. Changed to `except Exception as exc` with a `logger.debug` line recording the instance URL, exception type, and message to aid diagnosis of mirror connectivity issues.

### Fixed: TextMerger Split Reliability

- **`split_merged_result` silent data loss on mismatch** (`text_merger.py`): When the translator returned a different number of segments than expected, the function continued with a misaligned `lines` list and silently paired translated text to wrong keys. Now: if `len(lines) > expected_count` the surplus is trimmed; if `len(lines) < expected_count` the missing slots are padded with the original untranslated text, guaranteeing a 1-to-1 mapping and preventing key/value drift.
- **`split_merged_result_checked` double `_split_lines` call** (`text_merger.py`): The method called `_split_lines` a second time after `split_merged_result` already called it internally, doubling work on every checked split. Refactored to a single call.
- **`_split_lines` blank-line bleed** (`text_merger.py`): Leading or trailing separators in the translated block produced empty strings in the `lines` list that shifted all subsequent key assignments by one. Added `if l.strip()` filter to drop blank entries.

### Improved: Plugin & Parser Coverage

- **Legacy Japanese boolean sentinel detection** (`json_parser.py`): Pre-2017 Japanese MV plugins commonly omit `@type boolean` but use `@default はい` / `@default いいえ` as boolean sentinels — the plugin code `eval()`s the parameter value against `var はい = true; var いいえ = false;` variables or uses direct string comparisons like `=== '右クリック'`. Two guard layers added: (1) `_is_technical_string()` now blocks bare `はい` / `いいえ` values by content; (2) `_is_metadata_defined_technical_param()` now blocks any parameter whose `@default` matches `LEGACY_BOOL_DEFAULTS` (`はい`, `いいえ`, `true`, `false`, `on`, `off`) even when `@type` is absent. `LEGACY_BOOL_DEFAULTS` exposed as a class-level frozenset constant.
