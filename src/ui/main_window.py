...
class MainWindow(MSFluentWindow):
    ...
    def start_pipeline(self, data: dict):
        ...
        self.pipeline.finished.connect(self.on_finished, Qt.ConnectionType.QueuedConnection)
        self.pipeline.stage_changed.connect(self.on_stage_changed, Qt.ConnectionType.QueuedConnection)
        self.pipeline.progress_updated.connect(self.on_progress, Qt.ConnectionType.QueuedConnection)
        self.pipeline.log_message.connect(self.on_log_message, Qt.ConnectionType.QueuedConnection)
        self.pipeline.finished.connect(self.thread.quit, Qt.ConnectionType.QueuedConnection)
        self.pipeline.finished.connect(self.pipeline.deleteLater, Qt.ConnectionType.QueuedConnection)
        self.thread.finished.connect(self._cleanup_thread, Qt.ConnectionType.QueuedConnection)
